...
_dl_map_segment_default_thp_pagesize (void)
{
  if (DL_MAP_DEFAULT_THP_PAGESIZE != 0)
    return DL_MAP_DEFAULT_THP_PAGESIZE;
...
{
  static enum hugepages_thp_mode_t thp_mode = hugepages_thp_mode_not_supported;
  static unsigned long int thp_pagesize;

  if (TUNABLE_GET (glibc, elf, hugetlb, int32_t, NULL) == 0)
    return p_align_max;

  if (__glibc_unlikely (thp_mode == hugepages_thp_mode_not_supported
			|| thp_pagesize == 0))
    {
      thp_mode = __hugepages_thp_mode ();
      thp_pagesize = _dl_map_segment_default_thp_pagesize ();
    }
  ...
  /* Aligning load segments that are large enough to the PMD size helps
     ...
     huge pages). */
  if (thp_mode == hugepages_thp_mode_always && thp_pagesize <= MAX_THP_PAGESIZE
